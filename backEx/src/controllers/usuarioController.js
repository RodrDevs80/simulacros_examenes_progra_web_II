...
const getAllUsuariosAdministradores = async (req, res) => {
  try {
    const allUsuariosAdmin = await Usuario.scope("administradores").findAll();
    if (allUsuariosAdmin.length === 0) {
      res.status(200).json({ status: 200, message: 'No hay usuarios administradores en la base de datos!!!' })
    } else {
      res.status(200).json(allUsuariosAdmin);
    }
  } catch (err) {
    res.status(500).json({ error: "Error al obtener el listado de usuarios administradores", message: err.message });
  }
}
...
export { getAllUsuario, getAllUsuariosActivos, getAllUsuariosAdministradores, getUsuarioById, createUsuario, updateUsuario, deleteUsuario, deleteLogicoUsuario }